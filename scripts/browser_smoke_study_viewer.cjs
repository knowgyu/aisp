...
(async () => {
  ...
  try {
    ...
    if (!horizontallySeparated || !sameVerticalBand || !substantialWidths) {
    ...
    if (/allow-scripts|allow-same-origin/.test(sandbox)) throw new Error(`unsafe sandbox attribute: ${sandbox}`);
